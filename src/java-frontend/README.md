...
This compiler is deliberately narrow. It currently emits default constructors,
void methods, `String[]` parameters, and `System.out.println` literal calls. The
backend uses the existing repository Jasmin assembly/classfile writer rather than
shelling out to `javac`. Unsupported Java constructs fail in strict compile mode
and remain visible as partial sidecars in tolerant pass-manager mode.
