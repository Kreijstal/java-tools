...
assembly/classfile writer only; it never shells out to `javac`. For broad corpus
builds, the default CLI mode keeps unsupported constructs visible in the result
metadata while emitting valid frontend-generated bytecode stubs for unsupported
method bodies and `Object` erasure for unsupported recovered type shapes. Pass
`--strict` to restore fail-fast behavior for unsupported constructs.
