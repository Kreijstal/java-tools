...
class JitCompiler {
  ...
  compileInlinePrimitiveLoopRegions(method) {
    ...
    // Large decoders often contain a call-bearing setup phase followed by
    // scalar-bounded float transforms. Keep the outer method canonical, but
    // lower one verified natural loop as an ordinary structured helper. This
    // avoids placing the numeric body inside an oversized generator while a
    // runtime trip guard retains scheduler fairness for unusual inputs.
    if (!plans.length) {
      const scalarFloatAllowed = new Set([
        ...allowed,
        "fload", "fload_0", "fload_1", "fload_2", "fload_3",
        "fstore", "fstore_0", "fstore_1", "fstore_2", "fstore_3",
        "fconst_0", "fconst_1", "fconst_2", "fadd", "fsub", "fmul",
        "fdiv", "frem", "fneg", "i2f", "faload", "fastore", "ldc",
        "ldc_w",
      ]);
      const candidates = [];
      for (let backedge = 0; backedge < items.length; backedge += 1) {
      ...
      candidates.sort((left, right) =>
        right.floatTraffic - left.floatTraffic ||
        (right.backedge - right.header) - (left.backedge - left.header));
      ...
      if (candidate) {
        ...
        syntheticItems[0] = {
          ...syntheticItems[0],
          instruction: {
            op: "goto", arg: items[header].labelDef.slice(0, -1),
          },
        };
        const syntheticMethod = {
          ...method,
          name: `${method.name}$inlineFloatLoop${header}`,
          descriptor: "()V",
          flags: ["private", "static"],
          accessFlags: 0x000a,
          attributes: [{
        ...
        this.normalizedCodeItemsCache.set(syntheticMethod, syntheticItems);
        const generated = this.structuredSsa.compile(syntheticMethod);
        if (generated && !generated.jvmStructuredContinuation) {
          const id = this.inlineLoopRegions.length;
          this.inlineLoopRegions.push({
