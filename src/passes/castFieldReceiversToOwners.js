...
function runCastFieldReceiversToOwners(astRoot, options = {}) {
  ...
  for (const cls of astRoot.classes || []) {
    if (classes.size > 0 && !classes.has(cls && cls.className)) continue;
    for (const item of cls.items || []) {
      if (!item || item.type !== 'method' || !item.method) continue;
      for (const attr of item.method.attributes || []) {
        const code = attr && attr.type === 'code' && attr.code;
        if (!code || !Array.isArray(code.codeItems)) continue;
        rewrites += castCode(code, { ...options, currentClass: cls && cls.className });
      }
    }
  }
...
function castCode(code, options = {}) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    if (op(items[i]) === 'getfield') {
      const insertion = receiverCastInsertion(items, i, stack, options);
...
function receiverCastInsertion(items, getfieldIndex, stack, options) {
  const ref = arg(items[getfieldIndex]);
  const owner = Array.isArray(ref) && typeof ref[1] === 'string' ? ref[1] : null;
  if (!isConcreteCastOwner(owner)) return null;
  const receiver = stack.length > 0 && stack[stack.length - 1] && stack[stack.length - 1].kind === 'local'
    ? stack[stack.length - 1]
    : findReceiverLoadBackward(items, getfieldIndex);
  if (!receiver || receiver.kind !== 'local') return null;
  if (receiver.local === '0' && owner === options.currentClass) return null;
  const afterProducer = nextInstructionIndex(items, receiver.index);
  if (afterProducer < 0 || afterProducer > getfieldIndex) return null;
  if (op(items[afterProducer]) === 'checkcast' && arg(items[afterProducer]) === owner) return null;
  return { index: afterProducer, owner };
}

function findReceiverLoadBackward(items, getfieldIndex) {
  const prev = previousInstructionIndex(items, getfieldIndex);
  if (prev < 0 || (items[prev] && items[prev].labelDef)) return null;
  const local = aloadLocal(items[prev]);
  return local == null ? null : { kind: 'local', index: prev, local };
}
...
function applyStackEffect(stack, item, index) {
  ...
  if (itemOp === 'getfield') {
    ...
    return push({ kind: 'value', index, slots: categorySize(desc) });
  }
  ...
  if (/^invoke/.test(itemOp || '')) return applyInvokeStackEffect(stack, itemOp, item, push, pop, index);
...
function applyInvokeStackEffect(stack, itemOp, item, push, pop, index) {
  const ref = arg(item);
  const desc = Array.isArray(ref) && Array.isArray(ref[2]) ? ref[2][1] : null;
