...
class DebugJVM {
  ...
  loadClass(classFilePath, options = {}) {
    try {
      const rawClassData = loadClassByPath(classFilePath);
      
      // Transform the data structure to match what our JVM expects
      if (rawClassData && rawClassData.classes && rawClassData.classes.length > 0) {
        const classInfo = rawClassData.classes[0];
        const transformedClass = {
          name: classInfo.className,
          methods: classInfo.items
            .filter(item => item.type === 'method')
            .map(item => this.transformMethod(item.method))
        };
        
        this.classes[transformedClass.name] = transformedClass;
  ...
  transformMethod(method) {
    ...
    // Transform the code items to the expected format
    const transformedCodeItems = codeAttr.code.codeItems
      .filter(item => item.instruction !== null)
      .map(item => {
        if (typeof item.instruction === 'string') {
          return [item.instruction];
        } else if (item.instruction && item.instruction.op) {
          return [item.instruction.op, item.instruction.arg];
        }
        return ['nop']; // fallback
      });
    ...
    return {
      ...
      attributes: [{
        type: 'code',
        code: {
          localsSize: codeAttr.code.localsSize,
          codeItems: transformedCodeItems,
          exceptionTable: codeAttr.code.exceptionTable || []
        }
      }]
    };
  }
