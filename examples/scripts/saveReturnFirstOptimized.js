...
function main() {
  ...
  const { stages } = runOptimizationPipeline(program);
  const firstInlineStage = stages.find((stage) => stage.name === 'inline');
  if (!firstInlineStage || !firstInlineStage.changed) {
    console.warn('Inlining reported no changes for the ReturnFirst demo.');
  }
  const firstDeadCodeStage = stages.find((stage) => stage.name === 'deadCode');
  if (!firstDeadCodeStage || !firstDeadCodeStage.changed) {
